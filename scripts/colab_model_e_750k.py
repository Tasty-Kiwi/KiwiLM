"""Matched-budget Model E training workload executed non-interactively on Colab."""
...
EXPECTED_PARAMETER_COUNT = 6_050_816
DATA_DIR = Path("/content/kiwilm-model-e-750k/data")
RUN_DIR = Path("/content/kiwilm-model-e-750k/run")
SUITE = Path("/content/story-consistency-prompts.json")
EXAMPLES = Path("/content/kiwilm-model-e-750k-examples.md")
SUMMARY = Path("/content/kiwilm-model-e-750k-summary.json")
...
from kiwilm.config import CNNInterleavedAttentionConfig  # noqa: E402
...
validate_data(data)
model_config = CNNInterleavedAttentionConfig(vocab_size=data.tokenizer.vocab_size)
parameter_count = sum(
    parameter.numel() for parameter in build_model(model_config).parameters()
)
if parameter_count != EXPECTED_PARAMETER_COUNT:
    raise RuntimeError(
        f"expected {EXPECTED_PARAMETER_COUNT} Model E parameters, "
...
report_summary = generate_example_report(
    ...
    title=(
        "Model E Matched Model F Data "
        "(trained on 750k stories, 26.5:1 ratio) Examples"
    ),
)
generated = run(
    "kiwilm",
