...
session_name="${COLAB_SESSION_NAME:-kiwilm-model-b-750k}"
colab_bin="${COLAB_BIN:-colab}"
data_dir="${KIWILM_DATA_DIR:-data/tinystories-750k}"
artifact_dir="$(mktemp -d "${TMPDIR:-/tmp}/kiwilm-model-b-750k.XXXXXX")"
result_dir="${KIWILM_RESULT_DIR:-runs/model-b-750k-colab}"
...
"${colab_bin}" exec -s "${session_name}" \
  --timeout 14400 \
  -f scripts/colab_model_b_750k.py

"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-b-750k/run/best.pt \
  "${result_dir}/best.pt"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-b-750k/run/latest.pt \
  "${result_dir}/latest.pt"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-b-750k/run/metrics.jsonl \
  "${result_dir}/metrics.jsonl"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-b-750k-summary.json \
  "${result_dir}/summary.json"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-b-750k-examples.md \
  "${result_dir}/examples.md"

echo "Model B 750k artifacts downloaded to ${result_dir}"
