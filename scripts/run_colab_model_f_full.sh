...
session_name="${COLAB_SESSION_NAME:-kiwilm-model-f-full}"
colab_bin="${COLAB_BIN:-colab}"
data_dir="${KIWILM_DATA_DIR:-data/tinystories-750k}"
artifact_dir="$(mktemp -d "${TMPDIR:-/tmp}/kiwilm-model-f-full.XXXXXX")"
result_dir="${KIWILM_RESULT_DIR:-runs/model-f-colab}"
...
"${colab_bin}" exec -s "${session_name}" \
  --timeout 14400 \
  -f scripts/colab_model_f_full.py

"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-f-full/run/best.pt \
  "${result_dir}/best.pt"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-f-full/run/latest.pt \
  "${result_dir}/latest.pt"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-f-full/run/metrics.jsonl \
  "${result_dir}/metrics.jsonl"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-f-full-summary.json \
  "${result_dir}/summary.json"
"${colab_bin}" download -s "${session_name}" \
  /content/kiwilm-model-f-examples.md \
  "${result_dir}/examples.md"

echo "Model F artifacts downloaded to ${result_dir}"
