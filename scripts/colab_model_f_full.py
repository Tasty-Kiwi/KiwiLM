"""Full Model F 20:1 training workload executed non-interactively on Colab."""
...
DATA_DIR = Path("/content/kiwilm-model-f-full/data")
RUN_DIR = Path("/content/kiwilm-model-f-full/run")
SUITE = Path("/content/story-consistency-prompts.json")
EXAMPLES = Path("/content/kiwilm-model-f-examples.md")
SUMMARY = Path("/content/kiwilm-model-f-full-summary.json")
...
report_summary = generate_example_report(
    ...
    title=(
        "Model F Deep Interleaved Attention "
        "(trained on 750k stories, 20:1 ratio) Examples"
    ),
)
generated = run(
    "kiwilm",
