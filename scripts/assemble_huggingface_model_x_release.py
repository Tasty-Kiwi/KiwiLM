"""Assemble the standalone Model X Hugging Face release."""
...
def main() -> int:
    ...
    for source in sorted(args.bundle.iterdir()):
        if source.is_file():
            shutil.copy2(source, destination / source.name)
    shutil.copy2(
        args.repo_root / "docs/huggingface-model-x-card.md",
        destination / "README.md",
    )
    shutil.copy2(args.repo_root / "docs/model-x.svg", destination / "model-x.svg")
    ...
    evaluation_source = args.repo_root / "examples/comparisons/sft-v2-x-y"
    evaluation_destination = destination / "evaluation"
    evaluation_destination.mkdir()
    for name in ("report.md", "results.jsonl", "summary.json"):
    ...
    manifest = {
        ...
        "repository": "Tasty-Kiwi/KiwiLM-X",
        "files": files,
    }
